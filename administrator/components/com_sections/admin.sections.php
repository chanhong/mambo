...
function removeSections( $cid, $scope, $option ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('".T_('Select a section to delete')."'); window.history.go(-1);</script>\n";
		exit;
	}
...
function publishSections( $scope, $cid=null, $publish=1, $option ) {
	...
	if ( !is_array( $cid ) || count( $cid ) < 1 ) {
		...
		echo "<script> alert('".sprintf(T_('Select a section to '), $action)."'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if ( $publish ) {
		if ( !$count ){
			echo "<script> alert('".T_('Cannot Publish an Empty Section')."'); window.history.go(-1);</script>\n";
			return;
		}
	}
