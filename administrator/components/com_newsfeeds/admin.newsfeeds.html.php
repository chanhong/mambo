...
class HTML_newsfeeds {
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			<tr class="<?php echo 'row'. $k; ?>">
				...
				if ( $row->checked_out && ( $row->checked_out != $my->id ) ) {
					?>
					<?php echo $row->name; ?>
					&nbsp;[ <i><?php echo T_('Checked Out'); ?></i> ]
					<?php
				} else {
					?>
					<a href="<?php echo $link; ?>" title="<?php echo T_('Edit News Feed'); ?>">
					<?php echo $row->name; ?>
					</a>
					<?php
				}
	...
	function editNewsFeed( &$row, &$lists, $option ) {
		...
		function submitbutton(pressbutton) {
			...
			// do field validation
			if (form.name.value == '') {
				alert( "<?php echo T_('Please fill in the news feed name.'); ?>" );
			} else if (form.catid.value == 0) {
				alert( "<?php echo T_('Please select a Category.'); ?>" );
			} else if (form.link.value == '') {
				alert( "<?php echo T_('Please fill in the news feed link.'); ?>" );
			} else if (getSelectedValue('adminForm','catid') < 0) {
				alert( "<?php echo T_('Please select a category.'); ?>" );
			} else if (form.numarticles.value == "" || form.numarticles.value == 0) {
				alert( "<?php echo T_('Please fill in the number of articles to display.'); ?>" );
			} else if (form.cache_time.value == "" || form.cache_time.value == 0) {
				alert( "<?php echo T_('Please fill in the cache refresh time.'); ?>" );
			} else {
				submitform( pressbutton );
			}
		}
		...
		<tr>
			<th class="edit">
			<?php echo T_('News Feed:'); ?> <small><?php echo $row->id ? T_('Edit') : T_('New');?></small> <small><small>[ <?php echo $row->name;?> ]</small></small>
			</th>
		</tr>
		</table>
		...
		<tr>
			<td>
			<?php echo T_('Number of Articles'); ?>
			</td>
			<td>
		...
		<tr>
			<td>
			<?php echo T_('Cache time (in seconds)'); ?>
			</td>
			<td>
