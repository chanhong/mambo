...
function viewContent( $sectionid, $option ) {
	...
	if ( $sectionid == 0 ) {
		...
		$section->title = T_('All Content Items');
		$section->id = 0;
	} else {
		$where = array(
		"c.state >= 0",
...
function viewArchive( $sectionid, $option ) {
	...
	$authors[] = mosHTML::makeOption( '0', T_('- All Authors -') );
	$database->setQuery( $query );
	$moreauthors = $database->loadObjectList();
	if ($moreauthors) $authors = array_merge( $authors, $moreauthors );
	$lists['authorid']	= mosHTML::selectList( $authors, 'filter_authorid', 'class="inputbox" size="1" onchange="document.adminForm.submit( );"', 'value', 'text', $filter_authorid );
...
function changeContent( $cid=null, $state=0, $option ) {
	...
	if ( $state == "-1" ) {
		$msg = sprintf(Tn_('%d Item successfully Archived', '%d Items successfully Archived',$total),$total);
	} else if ( $state == "1" ) {
...
function moveSectionSave( &$cid, $sectionid, $option ) {
	...
	if (!$newsect && !$newcat ) {
		mosRedirect( "index.php?option=com_content&sectionid=$sectionid&mosmsg=".T_('An error has occurred')."" );
	}
	...
	$msg = sprintf(Tn_('%d Item successfully moved to Section: %s, Category: %s', '%d Items successfully moved to Section: %s, Category: %s',$total),$total, $section, $category);
