	...
	*/
	function showContent( &$rows, $section, &$lists, $search, $pageNav, $all=NULL, $redirect ) {
		...
		<tr>
			...
			if ( $all ) {
				?>
				<?php echo T_('Content Items Manager'); ?> <small><small>[ <?php echo T_('Section: All'); ?> ]</small></small>
				<?php
			} else {
				?>
				<?php echo T_('Content Items Manager'); ?> <small><small>[ <?php echo T_('Section:'); ?> <?php echo $section->title; ?> ]</small></small>
				<?php
			}
			?>
			</th>
	...
	*/
	function showArchive( &$rows, $section, &$lists, $search, $pageNav, $option, $all=NULL, $redirect ) {
		...
		function submitbutton(pressbutton) {
			if (pressbutton == 'remove') {
				if (document.adminForm.boxchecked.value == 0) {
					alert('<?php echo T_('Please make a selection from the list to send to Trash'); ?>');
				} else if ( confirm('<?php echo T_('Are you sure you want to Trash the selected items? \nThis will not permanently delete the items.'); ?>')) {
					submitform('remove');
				}
			} else {
		...
		<tr>
			...
			if ( $all ) {
				?>
				<?php echo T_('Archive Manager'); ?> <small><small>[ <?php echo T_('Section:'); ?> <?php echo T_('All'); ?> ]</small></small>
				<?php
			} else {
				?>
				<?php echo T_('Archive Manager'); ?> <small><small>[ <?php echo T_('Section:'); ?> <?php echo $section->title; ?> ]</small></small>
				<?php
			}
			?>
			</th>
	...
	function editContent( &$row, $section, &$lists, &$sectioncategories, &$images, &$params, $option, $redirect, &$menus ) {
		...
		<tr>
			...
			if ( $row->id ) {
				?>
				<small><small>
				[ <?php echo T_('Section:'); ?> <?php echo $section?> ]
				</small></small>
				<?php
			}
			?>
		...
		<tr>
			<td valign="top">
				...
				<tr>
					<td width="100%">
					<?php echo T_('Intro Text:'); ?> (<?php echo T_('required'); ?>)
					<br /><?php
				...
				<tr>
					<td width="100%">
					<?php echo T_('Main Text:'); ?> (<?php echo T_('optional'); ?>)
					<br /><?php
					// parameters : areaname, content, hidden field, width, height, rows, cols
				...
				<tr>
					<td width="200">
						...
						<tr>
							...
							<tr>
								<td valign="top" align="right">
								<?php // cause Mambo to be extremely slow w/ huge user table, comment out for now echo T_('Change Creator:'); ?>
								</td>
								<td>
								<?php // cause Mambo to be extremely slow w/ huge user table, comment out for now echo $lists['created_by']; ?> </td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td>
								<?php echo $row->version;?> <?php echo T_('times'); ?>
								</td>
							</tr>
							<tr>
								<td valign="top" align="right">
								...
								<td>
								<?php echo $row->created ? "$create_date</td></tr><tr><td valign='top' align='right'><strong>".T_('By')."</strong></td><td>$row->creator" : "New document"; ?>
								</td>
							</tr>
							<tr>
								<td valign="top" align="right">
								<strong>
								<?php echo T_('Last Modified'); ?>
								</strong>
								</td>
								<td>
								<?php echo $row->modified ? "$mod_date</td></tr><tr><td valign='top' align='right'><strong>".T_('By')."</strong></td><td>$row->modifier" : "Not modified";?>
								</td>
							</tr>
							</table>
							<?php
							...
							<tr>
								<td colspan="6"><?php echo T_('Sub-folder:'); ?> <?php echo $lists['folders'];?></td>
							</tr>
							<tr>
								<td>
							...
							<tr>
								<td>
								* <?php echo T_('These Parameters only control what you see when you click to view an item fully'); ?> *
								<br /><br />
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td>
								<input type="button" class="button" value="<?php echo T_('Add Sect/Cat/Title'); ?>" onClick="f=document.adminForm;f.metakey.value=document.adminForm.sectionid.options[document.adminForm.sectionid.selectedIndex].text+', '+getSelectedText('adminForm','catid')+', '+f.title.value+f.metakey.value;" />
								</td>
							</tr>
							</table>
							<?php
							...
							<tr>
								<td colspan="2">
								<?php echo T_("This will create a 'Link - Content Item' in the menu you select") ?>
								<br /><br />
								</td>
							</tr>
							<tr>
	...
	function moveSection( $cid, $sectCatList, $option, $sectionid, $items ) {
		...
		function submitbutton(pressbutton) {
			...
			if (!getSelectedValue( 'adminForm', 'sectcat' )) {
				alert( "<?php echo T_('Please select something'); ?>" );
			} else {
				submitform( pressbutton );
			}
		}
		...
		<tr>
			...
			<strong><?php echo T_('Move to Section/Category:'); ?></strong>
			<br />
			<?php echo $sectCatList; ?>
			<br /><br />
			</td>
			<td align="left" valign="top">
			<strong><?php echo T_('Items being Moved:'); ?></strong>
			<br />
			<?php
			echo "<ol>";
	...
	*/
	function copySection( $option, $cid, $sectCatList, $sectionid, $items  ) {
		...
		function submitbutton(pressbutton) {
			...
			// do field validation
			if (!getSelectedValue( 'adminForm', 'sectcat' )) {
				alert( "<?php echo T_('Please select a Section/Category to copy the items to'); ?>" );
			} else {
				submitform( pressbutton );
			}
		}
		...
		<tr>
			<th class="edit">
			<?php echo T_('Copy Content Items'); ?>
			</th>
		</tr>
		</table>
		...
		<tr>
			<td align="left" valign="top" width="40%">
			<strong><?php echo T_('Copy to Section/Category:'); ?></strong>
			<br />
			<?php echo $sectCatList; ?>
			<br /><br />
			</td>
			<td align="left" valign="top">
			<strong><?php echo T_('Items being copied:'); ?></strong>
			<br />
			<?php
			echo "<ol>";
			foreach ( $items as $item ) {
