	...
	*/
	function showCategories( &$params, &$categories, $catid ) {
		...
		if ($categories)  {
			...
			echo T_('There are currently no links');
		}
		?>
		</ul>
		<?php
	...
	*/
	function editWeblink( $option, &$row, &$lists ) {
		...
		function submitbutton(pressbutton) {
			...
			// do field validation
			if (form.title.value == ""){
				alert( '<?php echo T_("Web Link item must have a title"); ?>' );
		...
		<tr>
			...
			<td width="80%">
			<input class="inputbox" type="text" name="title" size="50" maxlength="250" value="<?php echo htmlspecialchars( stripslashes($row->title, ENT_QUOTES ));?>" />
			</td>
		</tr>
		<tr>
