...
function saveUpload($database, $_dbprefix, $uid, $option, $userfile, $userfile_name, $type, $existingImage) {
	...
	if ($checksize > 50000) {
		echo "<script> alert(\"".T_('You cannot upload files greater than 15kb in size.')."\"); window.history.go(-1); </script>\n";
	} else {
		if (file_exists($base_Dir.$userfile_name)) {
			$message = sprintf(T_('Image %s already exists. Please rename the file and try again.'),$userfile_name);
			...
			if ((!strcasecmp(substr($userfile_name,-4),".gif")) || (!strcasecmp(substr($userfile_name,-4),".jpg"))) {
				...
				{
					printf(T_('Failed to copy %s'), $userfile_name);
				} else {
				...
				echo "<script> alert(\"".T_('You may only upload a gif or jpg image.')."\"); window.history.go(-1); </script>\n";
			}
		}
	}
}
...
?>
<tr>
	<td colspan="2"><strong><?php echo T_('All items checked out have now been checked in'); ?></strong></td>
</tr>
</table>
<?php
}
