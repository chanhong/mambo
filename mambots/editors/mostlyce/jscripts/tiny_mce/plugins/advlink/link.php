...
require_once('../../../../../../../components/com_content/content.class.php');

$configuration =& mamboCore::getMamboCore();
$database =& mamboDatabase::getInstance();
$mainframe =& new contentHandler();
...
if (is_array($rows)) {	
	...
	$list['article'] = "<select name=\"articles\" $javascript >";
	$list['article'] .= "<option value=\"\">Select Article</option>";
	
	foreach ( $rows as $row ) { // get Itemid	
		$Itemid = $mainframe->getItemid( $row->id, 0, 0, $bs, $bc, $gbs );
	
		// Blank itemid checker for SEF
		if ($Itemid == NULL) $Itemid = '';
		else $Itemid = '&amp;Itemid='. $Itemid;		
		$href = sefRelToAbs( 'index.php?option=com_content&amp;task=view&amp;id='. $row->id . $Itemid );
				
		$list['article'] .= "<option value=\"". $href ."\">". $row->title."</option>";
	}
			...
			<div id="general_panel" class="panel current">
				<fieldset>
					...
					<table border="0" cellpadding="4" cellspacing="0">
						...
						<tr>
							<td><label for="articlelist">Articles</label></td>
							<td colspan="2" id="articlelist"><?php echo $list['article']; ?></td>
						</tr>
