...
function makeAttrib2(attrib, value) {
	var formObj = document.forms[0];
	var valueElm = formObj.elements[attrib];
...
function insertAction() {
	...
	// obtain file type	
	var extn = src.substr(src.lastIndexOf(".")+1,src.length-src.lastIndexOf("."));  
	if (extn == "pdf" || extn == "rtf" || extn == "doc" || extn == "wmv" || extn == "xls") {
		type = "file";	
	} else {
		type = "image";
	}
	...
	if (elm != null && elm.nodeName == "IMG") {
		...
		tinyMCEPopup.execCommand("mceInsertContent", false, html);
			...
			var html = "<img src='mambots/editors/mostlyce/jscripts/tiny_mce/filemanager/images/icons/"+extn+".gif' width='12' height='12' alt='"+extn+"'>";
			html += "<a ";
			html += makeAttrib('href', convertURL(src, tinyMCE.imgElement));
		  html += makeAttrib('title');
			html += ">";
			html += formObj.alt.value;
			html += "</a>";	
			tinyMCEPopup.execCommand("mceInsertContent", false, html);
	}
...
function showPreviewImage(src, start) {
	...
	// v1
	var src1 = src.match(/([^\/]+)\.\w+$/)[1];
	
	//v2
	//var slashPos=src.lastIndexOf("/");
	//var dotPos=src.lastIndexOf(".");
	//alert(slashPos + " --- " + dotPos);
	//var ext=src.substr(dotPos+1,src.length-dotPos);     
	//var src1=src.substr(slashPos+1,(dotPos-1-slashPos));
	...
	if (src == "")
		...
		getImageData(src)
		document.forms[0].alt.value = '' + src1		
		document.forms[0].title.value = '' + src1;
